# Summary

[Introduction](README.md)
[Why tma](explanation/why-tma.md)

# Tutorial

- [Getting started](tutorial/getting-started.md)

# How-to guides

- [Install tma](how-to/install-tma.md)
- [Install agent hooks](how-to/install-agent-hooks.md)
- [Add a custom agent](how-to/add-a-custom-agent.md)
- [Run an agent in a container](how-to/agents-in-containers.md)
- [Run tma over ssh](how-to/run-tma-over-ssh.md)
- [Set up notifications](how-to/notifications.md)
- [Author a custom action](how-to/custom-actions.md)
- [Block a script on agent state](how-to/block-a-script-on-agent-state.md)
- [Stream state changes](how-to/stream-state-changes.md)
- [Run the daemon](how-to/run-the-daemon.md)
- [Show agents in your status line](how-to/show-agents-in-your-status-line.md)
- [Drive an external bar](how-to/drive-an-external-bar.md)
- [Install the keybindings](how-to/install-the-keybindings.md)
- [Diagnose with tma doctor](how-to/diagnose-with-doctor.md)
- [Read agent state from a status bar or script](how-to/read-agent-state-from-a-status-bar-or-script.md)

# Reference

- [Command-line interface](reference/cli.md)
- [Keybindings](reference/keybindings.md)
- [Configuration](reference/configuration.md)
- [Pane options and JSON contracts](reference/pane-options-and-json.md)
- [The `@agent_state` contract](reference/agent-state-contract.md)
- [Agent coverage](reference/agent-coverage.md)
- [Manifest schema](reference/manifest-schema.md)
- [Action manifest schema](reference/action-manifest-schema.md)

# Explanation

- [Architecture](explanation/architecture.md)
- [The detection model](explanation/detection-model.md)
- [The security model](explanation/security-model.md)
