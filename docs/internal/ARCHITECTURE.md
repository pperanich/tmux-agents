...
- Consolidated option schema (supersedes the PRD table). Adversarial review exposed
  ...
  | `@tma_title_match_pid` | pane | flicker-stickiness anchor, stored as `<agent>:<pid>`: the title-narrowed manifest that last matched `#{pane_title}` and the agent pid it matched on. The identity resolver holds the title match for THAT manifest while the pane's agent pid is unchanged; a new pid re-requires a match, and another manifest walking the same pid (cursor, gemini and pi all match a bare `node`) gets no hold from it. A bare pid, the format before the agent name was added, holds nothing. Written only for a `title_patterns` manifest, never for a process-only one |
