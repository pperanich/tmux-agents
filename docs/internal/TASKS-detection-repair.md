...
**A1 — delete the three producers.** `OPEN`
...
**A2 — retire the `Source` variant, keep the `Provenance`.** `OPEN`
...
**A3 — re-source the affected tests (mechanical, no coverage lost).** `OPEN`
...
**A4 — replace, do not delete, the ordering test.** `OPEN`
...
**A5 — amend the normative docs.** `OPEN`
