...
| `--check` | Verify the managed file is current and the resolved tmux config sources it exactly once; report drift. A file with or without the mouse group counts as current. |
