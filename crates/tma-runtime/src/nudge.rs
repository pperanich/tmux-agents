...
//! option dies with its pane, so only a watcher killed without unsetting it leaves a residual window.
...
/// Install the SIGUSR1 handler for a resident watcher (via signal-hook). Idempotent. signal-hook
...
/// Signal every resident `tma watch` to refresh (the sender): walk panes for
/// `@tma_watch_pid` and SIGUSR1 each pid that parses and is `> 0`. Best-effort; a gone server or a
/// stale option (ESRCH) is tolerated silently, and every advertiser is signalled (multiple watchers
/// are legal).
...
/// zero/negatives (`kill(0/-1, …)` fans out to process groups) and non-numeric junk.
fn parse_watch_pid(raw: &str) -> Option<i32> {
